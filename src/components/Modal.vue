<template>
  ...
  >
    ...
    <div
      :class="`modal-container  ${customContainerClass}`"
      :style="{
        width: props.width,
        paddingTop: isCustomWrapper ? '0px' : '',
        overflowY: disableVerticalOverFlow ? 'hidden' : 'auto',
      }"
    >
      <div
        :class="customerWrapper ? 'modal-content-container-hiddenScroll' : ''"
      >
...
<script setup>
  ...
  const props = defineProps({
    ...
    subHeader: {
      type: String,
      default: "Please provide the following details",
    },
