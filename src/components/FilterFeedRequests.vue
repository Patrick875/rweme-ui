    ...
    <a-form :model="feedReqFilterForm" name="basic" :label-col="{ span: 24 }">
      <a-row :gutter="[16, 16]">
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              v-model:value="feedReqFilterForm.deliveryStatus"
              :size="'middle'"
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              v-model:value="feedReqFilterForm.typeOfFeed"
              :options="typesOfFeed"
              :size="'middle'"
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          <a-form-item class="label-input-height" label="Farmer" name="farmer">
            <a-select
              v-model:value="feedReqFilterForm.farmer"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              <a-select-option
                v-for="farm in farmers"
                :key="farm.id"
                :value="farm.key"
              >
                {{ farm.label }}
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              v-model:value="feedReqFilterForm.supplier"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              <a-select-option
                v-for="supl in suppliers"
                :key="supl.id"
                :value="supl.key"
              >
                {{ supl.label }}
...
<script setup lang="ts">
  ...
  const farmers = computed(() =>
    entitiesStore.farmers?.map((farmer) => ({
      id: farmer.id,
  ...
  const filter = async () => {
    ...
    const cleanFilterObj = Object.keys(feedReqFilterForm.value).reduce(
      (acc, key) => {
        if (
          feedReqFilterForm.value[key] != undefined &&
          feedReqFilterForm.value[key] != null &&
          feedReqFilterForm.value[key] != ""
        ) {
