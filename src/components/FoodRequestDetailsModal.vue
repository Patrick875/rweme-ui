      ...
      <template #content>
        <div v-if="!props.loading && props.viewableFoodRequest">
          ...
          <div class="fd-req-detail">
            ...
            <p>
              {{
                viewableFoodRequest ? viewableFoodRequest?.TypeOfFeed?.name : ""
              }}
            </p>
          </div>
          <div class="fd-req-detail">
            ...
            <p>
              {{
                viewableFoodRequest ? viewableFoodRequest?.quantityOfFeed : ""
              }}
            </p>
          </div>
          <div class="fd-req-detail">
            <p>Price of Feed:</p>
            <p>{{ viewableFoodRequest ? viewableFoodRequest?.price : "" }}</p>
          </div>
          <div class="fd-req-detail">
            <p>Submitted on:</p>
            <p>
              {{
                new Date(viewableFoodRequest?.createdAt).toLocaleString("fr-FR")
              }}
            </p>
          </div>
          <a-button
            v-if="
              viewableFoodRequest.deliveryStatus !== 'DELIVERED' &&
              userRole !== 'Admin'
            "
            class="process-button"
            @click="initiateRequestDelivery"
            >{{
              loading
                ? "...loading"
                : `INITIATE
...
<script setup lang="ts">
  ...
  const initiateRequestDelivery = async () => {
    loading.value = true;
    await instance
      .patch(`/foodrequests/${props.viewableFoodRequest.id}/initiatedelivery`, {
