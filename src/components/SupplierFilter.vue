      ...
      <a-row :gutter="[16, 16]">
        <a-col :span="isSmallScreen ? 24 : 12">
          <a-form-item class="label-input-height" label="Status" name="status">
            <a-select
              v-model:value="supFilterForm.status"
              :size="'middle'"
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              v-model:value="supFilterForm.supTypeOfFeeds"
              :options="typesOfFeed"
              mode="multiple"
              :size="'middle'"
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              v-model:value="supFilterForm.province"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              :disabled="!supFilterForm.province"
              v-model:value="supFilterForm.district"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              ...
              <a-select-option
                v-for="district in districts.filter(
                  (distr) => distr.provinceId === supFilterForm.province
                )"
                :key="district.id"
                :value="district.key"
              >
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              :disabled="!supFilterForm.district"
              v-model:value="supFilterForm.vetSector"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              ...
              <a-select-option
                v-for="sector in sectors.filter(
                  (sect) => sect.districtId === supFilterForm.district
                )"
                :key="sector.id"
                :value="sector.key"
              >
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          <a-form-item class="label-input-height" label="Cell" name="cell">
            <a-select
              :disabled="!supFilterForm.vetSector"
              v-model:value="supFilterForm.cell"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              ...
              <a-select-option
                v-for="cell in cells.filter(
                  (cell) => cell.sectorId === supFilterForm.vetSector
                )"
                :key="cell.id"
                :value="cell.key"
              >
        ...
        <a-col :span="isSmallScreen ? 24 : 12">
          ...
          >
            <a-select
              :disabled="!supFilterForm.cell"
              v-model:value="supFilterForm.village"
              :size="'middle'"
              placeholder="Please select !!"
            >
              <a-select-option value="">---</a-select-option>
              <a-select-option
                v-for="village in villages.filter(
                  (vill) => vill.cellId === supFilterForm.cell
                )"
                :key="village.id"
                :value="village.key"
              >
...
<script setup lang="ts">
  ...
  const filter = async () => {
    ...
    const cleanFilterObj = Object.keys(supFilterForm.value).reduce(
      (acc, key) => {
        if (
          supFilterForm.value[key] != undefined &&
          supFilterForm.value[key] != null &&
          supFilterForm.value[key] != ""
        ) {
          if (
            key === "supTypeOfFeeds" &&
            supFilterForm.value[key].length == 0
          ) {
            acc = acc;
          } else {
