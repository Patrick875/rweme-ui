<template>
  <div>
    <Modal
      :isOpen="props.isToggleFoodRequestModal"
      @modal-close="props.cancelButton"
      mainHeader="FOOD REQUEST"
        ...
        >
          <a-row :gutter="16">
            <a-col :span="isSmallScreen ? 24 : 12">
              ...
              >
                <a-select
                  :disabled="props.farmerId"
                  v-model:value="foodRequest.farmerId"
                  :size="'middle'"
                  placeholder="Please select"
                >
                  <a-select-option disabled value=""
                    >Select farmer</a-select-option
                  >
                  <a-select-option
                    v-for="(farmer, i) in farmers"
                    :value="farmer.key"
                    :key="i"
                    >{{ farmer.fullName + " / " + farmer.telephone }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="isSmallScreen ? 24 : 12">
              ...
              >
                <a-select
                  v-model:value="foodRequest.typeOfFeed"
                  :size="'middle'"
                  placeholder="Please select"
                >
                  <a-select-option disabled value=""
                    >Select food</a-select-option
                  >
                  <a-select-option
            ...
            <a-col :span="isSmallScreen ? 24 : 12">
              <a-form-item
                ...
                name="priceOfFeed"
                :rules="[
                  { message: 'Please fill in the price !', required: true },
                ]"
              >
                <a-input-number
                  :min="1"
            ...
            <a-col :span="isSmallScreen ? 24 : 12">
              ...
              >
                <a-select
                  v-model:value="foodRequest.supplierId"
                  :size="'middle'"
                  placeholder="Please select"
                >
                  <a-select-option disabled value=""
            ...
            <a-col :span="isSmallScreen ? 24 : 12">
              <p>
                Total to be paid:
                {{ Number(totalToBePaid).toLocaleString() + " RWF" }}
              </p>
            </a-col>
          </a-row>
          <div class="double-form-btn">
            <a-button
              class="cancel-form-btn model-action-btn"
              danger
              html-type="button"
            ...
            <a-button
              :loading="loading"
              class="btn-auth model-action-btn"
              @click="submitRequest"
...
<script setup lang="ts">
  ...
  const submitRequest = async () => {
    ...
    const submitData = {
      farmerId: foodRequest.value.farmerId,
      supplierId: foodRequest.value.supplierId,
      typeOfFeed: foodRequest.value.typeOfFeed,
      price: foodRequest.value.priceOfFeed,
      quantityOfFeed: foodRequest.value.quantityOfFeed,
      totalAmount: totalToBePaid.value,
      dateOfPayment:foodRequest.value.dateOfPayment
    };
