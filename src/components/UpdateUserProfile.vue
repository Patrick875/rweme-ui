  ...
  >
          ...
          >
            <a-select-option
              v-for="district in districts.filter(
                (distr) => distr.provinceId === formState.provinceId
              )"
              :key="district.id"
              :value="district.key"
            >
      ...
      <a-col :span="isSmallScreen ? 24 : 12">
        ...
        >
          <a-select
            ...
            v-model:value="formState.sectorId"
            :size="'middle'"
            placeholder="Please select !!"
          >
            <a-select-option
              v-for="sector in sectors.filter(
                (sect) => sect.districtId === formState.districtId
              )"
              :key="sector.id"
              :value="sector.key"
            >
      ...
      <a-col :span="isSmallScreen ? 24 : 12">
        ...
        >
          <a-select
            ...
            v-model:value="formState.cellId"
            :size="'middle'"
            placeholder="Please select !!"
          >
            <a-select-option
              v-for="cell in cells.filter(
                (cell) => cell.sectorId === formState.sectorId
              )"
              :key="cell.id"
              :value="cell.key"
            >
          ...
          >
            <a-select-option
              v-for="village in villages.filter(
                (vill) => vill.cellId === formState.cellId
              )"
              :key="village.id"
              :value="village.key"
            >
    ...
    <div class="double-form-btn g-flex">
      <a-button
        ...
        >CANCEL</a-button
      >
      <a-button :loading="loading" class="btn-auth" @click="updateMe"
        >UPDATE</a-button
      >
    </div>
  </a-form>
