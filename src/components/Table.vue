    ...
    <div class="table">
          ...
          <a-col v-if="!isReportsTable" :span="12">
            <div class="filter-section">
              <a-button
                class="btn-export"
                v-if="isShowFilter"
                @click="() => props.openFilter()"
                >Filter List</a-button
              >
              <a-button class="btn-export" @click="() => props.resetFilter()"
                >Reset</a-button
              >
              <a-dropdown>
                <template #overlay>
                  <a-menu @click="handleExport">
                    <a-menu-item
                      @click="() => props.handleExport(props.data, props.title)"
                      key="1"
                    >
                      Excel
                    </a-menu-item>
              ...
              <span>
                <span v-if="props.btnName">
                  <a-button
                    :disabled="isDisabled"
                    :class="!isDisabled ? 'btn-create' : 'btn-disabled'"
                    @click="() => props.handlePrimaryButtonClicks()"
                    >{{ btnName }}
                  </a-button>
                </span>
              </span>
      ...
      <a-table :columns="props.columns" :data-source="props.data">
        <template #bodyCell="{ column, record }">
          ...
          <template
            v-else-if="
              column.key === 'typeOfChicken' && column.title === 'Category'
            "
          >
          ...
          <template v-else-if="column.key === 'specializations'">
            <span>
              <a-tag
                v-for="tag in record.specializations"
                :key="tag"
                color="geekblue"
              >
          ...
          <template v-else-if="column.key === 'typeoffeeds'">
            <span>
              <a-tag
                v-for="tag in record.typeoffeeds"
                :key="tag"
                color="geekblue"
              >
                {{ tag.name.toUpperCase() }}
          ...
          <template v-else-if="column.key === 'status'">
            <span>
              <a-tag
                :color="
                  [accountStatus.active, 'Completed'].includes(record.status)
                    ? 'success'
                    : [
          ...
          <template v-else-if="column.key === 'action'">
            <div class="actions-column">
              <v-icon
                v-if="record.isFarmer || record.isFoodRequest"
                @click="onView(record)"
                class="view-icon-style"
                name="bi-eye"
              />
              <div
                @click="props.handleUpdateAction(record.id)"
                class="action-icon"
                data-caption="Edit"
              >
                <v-icon
                  v-if="record.status == accountStatus.active"
              ...
              <div class="action-icon" data-caption="Activate">
                <v-icon
                  @click="() => triggerUpdateStatus(record, 'Active')"
                  v-if="['Inactive', 'Blocked'].includes(record.status)"
                  class="active-style active-icon-style action-icon"
                  name="co-check-alt"
                />
              </div>
              <div class="action-icon" data-caption="Deactivate">
                <v-icon
                  @click="() => triggerUpdateStatus(record, 'Inactive')"
                  v-if="record.status == accountStatus.active"
                  class="deactivate-icon-style action-icon"
                  name="bi-eye-slash"
                />
              </div>
              <div
                v-if="props.userCanDelete"
                class="action-icon"
                data-caption="Delete"
              >
                <v-icon
                  @click="() => triggerDelete(record.id)"
                  v-if="record.status !== accountStatus.deleted"
                  class="deactivate-icon-style action-icon"
      ...
      <Modal
        :isOpen="isToggleStatusUpdateModal"
        @modal-close="() => (isToggleStatusUpdateModal = false)"
        ...
        <template #content>
          <div class="delete-btns">
            <a-button
              ...
              danger
              html-type="button"
              @click="() => (isToggleStatusUpdateModal = false)"
              >CANCEL</a-button
            >
            ...
            <a-button
              ...
              "
              >{{ newStatus == "Active" ? "Activate" : "Deactivate" }}</a-button
            >
          </div>
        </template>
      </Modal>
      <Modal
        :isOpen="isToggleDeleteModal"
        @modal-close="closeModal"
        mainHeader="DELETE"
        :subHeader="`Are you sure you want to delete this ${currentEntity}`"
        :width="isSmallScreen ? '80%' : '550px'"
      >
        <template #content>
          <div class="delete-btns">
