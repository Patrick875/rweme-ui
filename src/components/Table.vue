    ...
    <div class="table">
      <div class="table-header" v-if="!isSmallScreen">
        <a-row>
          ...
          <p v-if="isReportsTable" class="margin-text" style="text-align: center; font-weight: bold">
            {{ title }}
          </p>
              ...
              <a-dropdown>
                <template #overlay>
                  <a-menu @click="handleExport">
                    <a-menu-item @click="() => props.handleExport(props.data, props.title)" key="1">
                      Excel
                    </a-menu-item>
                  </a-menu>
                </template>
          ...
          <a-col v-if="!isReportsTable" :span="8">
            <div class="filter-section">
              <a-dropdown>
                <template #overlay>
                  <a-menu>
                    <a-menu-item key="1" v-if="isShowFilter" @click="() => props.openFilter()">
                      Filter
                    </a-menu-item>
                    <a-menu-item key="2" @click="() => props.resetFilter()">
                      Reset
                    </a-menu-item>
                    <a-menu-item key="3" @click="() => props.handleExport(props.data, props.title)">
                      Export
                    </a-menu-item>
      ...
      <a-table :columns="props.columns" :data-source="props.data">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'status'">
            <span>
              <a-tag :color="[accountStatus.active, 'Completed'].includes(record.status)
                ? 'success'
                : [
      ...
      <Modal :isOpen="isToggleStatusUpdateModal" @modal-close="() => (isToggleStatusUpdateModal = false)"
        ...
        <template #content>
          <div class="delete-btns">
            ...
            <a-button :loading="props.loading" class="btn-auth" @click="
              updateItemStatus(
                updateInfo.itemId,
                updateInfo.userId,
                updateInfo.status
