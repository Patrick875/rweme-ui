  ...
  <div>
    ...
    >
      <a-form-item
        ...
        name="auth"
        :rules="[
          { required: true, message: 'Please input your email/telephone!' },
        ]"
      >
      ...
      <a-button class="auth-btn" :loading="loading" @click="submit"
        >SUBMIT</a-button
      >
    </a-form>
  </div>
...
<script setup lang="ts">
  ...
  const submit = async () => {
    ...
    try {
      await instance
        .post("/auth/initiateForgotPassword", { login: formState.value.auth })
        .then((res) => {
