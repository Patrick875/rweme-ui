<template>
  <div>
    <Table
      :data="suppliers"
      :title="'Suppliers'"
      :length="String(suppliers.length)"
      :handle-table-search="handleSearch"
      :handleDeleteItem="deleteSupplier"
      :loading="loading"
      :handleUpdateItemStatus="updateSupplierStatus"
      :handle-update-action="handleUpdateSupplier"
      :handlePrimaryButtonClicks="handleCreateToggleSupplier"
      :btn-name="'Create Supplier'"
      :columns="columns"
    ...
    />
    <Modal
      :isOpen="isToggleUpdateVeternary"
      @modal-close="closeCreateSupplier"
      mainHeader="CREATE SUPPLIER"
    ...
    <Modal
      :isOpen="isToggleUpdateModal && supplier"
      @modal-close="() => (isToggleUpdateModal = false)"
      :mainHeader="'UPDATE SUPPLIER'"
      :subHeader="'Provide the following details to update the supplier'"
      :width="isSmallScreen ? '80%' : '550px'"
    >
      <template #content>
        <update-supplier
    ...
    <Modal
      :isOpen="isToggleFilterSuppliers"
      @modal-close="closeFilterSupModal"
      mainHeader="Filter"
...
<script setup lang="ts">
  ...
  const suppliers = computed(() =>
    entitiesStore.suppliers.map((item) => ({
      ...item,
  ...
  const updateSupplierStatus = async (data: any) => {
    loading.value = true;
    try {
      await entitiesStore.updateStatus(`/suppliers/status-update`, {
        ...data,
        supplierId: data.itemId,
      });
      entitiesStore.getSuppliers();
    } catch (error) {
      console.log("err", error);
  ...
  const exportToExcell = (data: Array<any>, entity: string) => {
    ...
    data.forEach((item) => {
      const rowData = [
        ...
        item.telephone || "",
        item.email || "",
        item.typeoffeeds.length > 0
          ? item.typeoffeeds.map((el) => el.name.toUpperCase()).join(", ")
          : "",
        item.momoPay || "",
        item.status || "",
      ];
