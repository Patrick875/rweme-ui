<template>
  <div class="admin-navbar">
    ...
    <a-dropdown v-if="!isSmallScreen">
      <template #overlay>
        <a-menu>
          <a-menu-item key="1" @click="openUpdateProfileModel"
            >Profile</a-menu-item
          >
          <a-menu-item @click="() => (openChangePassword = true)" key="2"
  ...
  <Modal
    ...
    @modal-close="() => (openChangePassword = false)"
    ...
    <template #content>
      ...
      >
        ...
        >
          <a-input-password
            class="input"
            v-model:value="changePasswordForm.comfirmPassword"
          />
        </a-form-item>
        <a-button
          class="auth-btn"
  ...
  <Modal
    ...
    @modal-close="closeUpdateProfileModel"
    :main-header="'UPDATE PROFILE'"
    subHeader="Fill the form to update your profile."
  >
    <template #content>
...
<script setup lang="ts">
  ...
  const changePassword = async () => {
    ...
    await instance
      .patch("/auth/changePassword", { ...changePasswordForm.value })
      .then(() => {
  ...
  const { resetFields } = Form.useForm(changePasswordForm, {
    oldPassword: [
      { required: true, message: "Please input your password!" },
      {
        ...
        message:
          "Password must be at least 8 characters long contain both letters,numbers and at least one special character.",
      },
    ],
    comfirmPassword: [
      { required: true, message: "Please confirm your password!" },
