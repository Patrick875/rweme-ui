<template>
  <div>
    <Table
      :data="farmers"
      :title="'Farmers'"
      :length="String(farmers.length)"
      :user-can-delete="!isVeternary"
      :loading="loading"
      :handleTableSearch="handleSearch"
      :handlePrimaryButtonClicks="handleCreateFarmer"
      :btn-name="'Add new farmer'"
      :columns="columns"
    ...
    />
    <Modal
      :isOpen="isToggleCreateFarmer"
      @modal-close="closeCreateFarmerModal"
      mainHeader="CREATE FARMER"
      subHeader="Fill the details to create a new farmer"
      :width="isSmallScreen ? '80%' : '550px'"
    >
      <template #content>
        <create-farmer-form
          :onComplete="next"
          :cancelButton="closeCreateFarmerModal"
        ></create-farmer-form>
      </template>
    </Modal>
    <Modal
      :isOpen="isToggleSignContract"
      @modal-close="closeCloseContractSignModal"
      mainHeader="SIGN CONTRACT"
      subHeader="click sign to sign the contract "
      :width="isSmallScreen ? '80%' : '550px'"
    >
      <template #content>
        <farmer-contract
          :loading="loading"
          :cancelButton="closeCloseContractSignModal"
          :sign="handleFarmerContract"
        ></farmer-contract>
      </template>
    </Modal>
    <Modal
      :isOpen="isToggleFilterFarmers"
      @modal-close="closeFilterFarmersModal"
      mainHeader="Filter"
    ...
    <Modal
      :isOpen="isToggleComfirmFarmer"
      @modal-close="closeComfirmFarmerModal"
      mainHeader=""
      subHeader=""
      :width="'550px'"
    >
