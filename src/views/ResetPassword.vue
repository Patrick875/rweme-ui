  ...
  <div>
    ...
    >
      ...
      >
        <a-input-password
          class="input"
          v-model:value="resetPasswordForm.comfirmPassword"
        />
      </a-form-item>
      <a-button
        class="auth-btn"
...
<script setup lang="ts">
  ...
  const resetPassword = async () => {
    ...
    const submitUrl = isSetPassword.value
      ? `/auth/set-password/${route.params.email}`
      : `/auth/forgotPassword/${route.params.email}/${route.params.token}`;
    await instance
      .post(`${submitUrl}`, {
      ...
      .then(() => {
        notify(
          "success",
          "Success",
          "Password reset succesfuly.Please procede to the login page."
        );
