  ...
  <a-tabs v-model:activeKey="activeKey">
    <a-tab-pane key="1" tab="Types Of Feed">
      <Table
        :data="typesOfFood"
        :title="'Types of feed'"
        :length="typesOfFood.length.toString()"
        :columns="columns"
        :handle-table-search="searchFeed"
        :handle-delete-item="deleteFeed"
        :handlePrimaryButtonClicks="() => (isCreateItem = true)"
        :btn-name="'Create type of feed'"
        :handle-export="exportToExcell"
      />
      <Modal
        :isOpen="isCreateItem"
        @modal-close="() => (isCreateItem = false)"
        mainHeader="CREATE FEED"
        subHeader="Please provide the following details"
    ...
    <a-tab-pane key="2" tab="Veternary Specializations">
      <Table
        ...
        :title="'Vet-Specializations'"
        :length="specializations.length.toString()"
        :columns="columns"
        :handle-table-search="searchSpecialization"
        :handle-delete-item="deleteSpecialization"
        :handlePrimaryButtonClicks="() => (isCreateItem = true)"
        :btn-name="'Create '"
        :handle-export="exportToExcell"
      />
      <Modal
        :isOpen="isCreateItem"
        @modal-close="() => (isCreateItem = false)"
        mainHeader="CREATE SPECIALIZATION"
        ...
        <template #content>
          ...
          >
            ...
            <div class="double-form-btn g-flex">
              ...
              >
              <a-button :loading="loading" class="btn-auth" html-type="submit"
                >CREATE SPECIALIZATION</a-button
              >
            </div>
          </a-form>
        </template>
    ...
    <a-tab-pane key="3" tab="Types Of Chicken">
      <Table
        :data="typesOfChicken"
        :title="'Types of chicken'"
        :length="typesOfChicken.length.toString()"
        :columns="columns"
        :handle-table-search="searchChicken"
        :handle-delete-item="deleteChickenType"
        :handlePrimaryButtonClicks="() => (isCreateItem = true)"
        :btn-name="'Create type'"
        :handle-export="exportToExcell"
      />
