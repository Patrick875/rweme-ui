<template>
  ...
  <a-tabs v-model:activeKey="activeKey">
      ...
      <div>
        ...
        <div class="farmer">
          ...
          <div>
            ...
            <div class="details-group">
              <p v-if="farmer">
                Current Number of Chicken : {{ farmer.numberOfChicken }}
              </p>
              <p v-if="farmer">
                Joined At :{{
      ...
      <div class="farmstatuses">
        <div class="farm-status-header">
          ...
          <div class="actions-btns">
            <a-button
              class="add-record"
              @click="() => (isToggleAddStatusModal = true)"
            >
        ...
        <div>
          <Table
            :title="'Farm records'"
            :length="String(farmRecords.length)"
            :data="farmRecords"
            :columns="columns"
          />
        </div>
      ...
      <div class="farmstatuses">
        <div class="farm-status-header">
          ...
          <div class="actions-btns">
            <a-button
              class="add-record"
              @click="() => (isToggleFoodRequestModal = true)"
            >
              <v-icon name="io-add-circle-outline"></v-icon>
        ...
        <Table
          :data="feedRequests"
          :title="'Feed Requests'"
          :length="String(feedRequests.length)"
          :handle-table-search="handleSearch"
          :handleViewDetails="openFoodRequest"
    ...
    <a-tab-pane key="4" tab="Contracts">
      <div v-if="farmerContracts.length > 0">
        <div
          v-for="(contract, i) in farmerContracts"
          :key="i + 1"
          class="contract-item"
        >
          <div class="contract-list-item">
            <v-icon
              ...
              name="vi-file-type-pdf2"
            ></v-icon>

            <p>
              {{
                `${farmer.fullName} _` + new Date(contract.createdAt).toString()
              }}
            </p>
          </div>
  ...
  <AddFarmStatusModel
    :isToggleAddStatusModal="isToggleAddStatusModal"
    :cancelButton="closeModal"
    :farmerId="farmerId"
    :chickenTypeId="chickenType"
  />
  <FoodRequestModal
    :cancelButton="() => (isToggleFoodRequestModal = false)"
    :isToggleFoodRequestModal="isToggleFoodRequestModal"
    :farmerId="farmerId"
  />
  <FoodRequestDetailsModal
    v-if="viewableFoodRequest"
    :cancelButton="() => (isViewFoodRequest = false)"
    :isToggleFoodRequestModal="isViewFoodRequest && viewableFoodRequest"
    :viewableFoodRequest="viewableFoodRequest.value"
    :viewableFoodRequestId="viewableItemId"
    :loading="loading"
...
<script setup lang="ts">
  ...
  const farmRecords = computed(() =>
    entitiesStore.farmer
      ? entitiesStore.farmer.FarmStatuses.map((el) => ({
          ...
          amountOfFeedOnDailyBasisPerChicken:
            el.amountOfFeedOnDailyBasisPerChicken,
  ...
  watchEffect(() => {
    if (farmer.value && typeOfChicken.value) {
      chickenType.value = typeOfChicken.value.find(
        (el) => el.id === farmer.value.TypeOfChickenId
      )?.id;
