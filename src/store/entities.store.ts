...
export const useEntitiesStore = defineStore({
  ...
  actions: {
    ...
    async getAppointments(q: string = "", filterString = "") {
      ...
      const url =
        filterString != ""
          ? `/apppointments?q=${q}&${filterString}`
          : `/appointments?q=${q}`;
      try {
        const response = await instance.get(`${url}`);
    ...
    async getRequestsByFarmer(farmerId: string, q: string = "") {
      ...
      try {
        const response = await instance.get(
          `/foodrequests/farmer/${farmerId}?q=${q}`
        );
        this.foodrequests = response.data.data;
      } catch (error) {
        console.log("err", error);
    ...
    async getRequestsBySupplier(supplierId: string, q: string = "") {
      ...
      try {
        const response = await instance.get(
          `/foodrequests/supplier/${supplierId}?q=${q}`
        );
        this.foodrequests = response.data.data;
      } catch (error) {
        console.log("err", error);
    ...
    async getContractDetails() {
      this.resetStatuses();
      try {
        const response = await instance.get(`/contract-details`);
        this.contractDetails = response.data.data;
      } catch (err) {
        console.log("err", err);
      }
    },
